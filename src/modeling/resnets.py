...
class DropoutBlock(nn.Module):
    ...
    def __init__(self, basic_block: BasicBlock, dropout_rate: float = 0.0):
...
def resnet_feature_extractor(resnet_model: Union[ResNet, ResNetCustomChannels]):
    """
    Makes the given resnet model support the feature extraction protocol
    by monkey patching it and removing the fully connected layer
    """
    feature_dim = resnet_model.fc.in_features
    resnet_model.fc = torch.nn.Identity()
    resnet_model.features_dim = feature_dim

    resnet_model.__class__.get_features = get_features
    return resnet_model
